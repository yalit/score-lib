...
export default function ScoreForm({score = null}: { score?: Score | null }) {
    ...
    const form = useForm<FormScore>({defaultValues: score ?? BlankScore, resolver: zodResolver(scoreFormSchema), mode: "onSubmit"});
    const {fields: otherReferences, append: appendReference, remove: removeReference} = useFieldArray({
        control: form.control,
        name: 'otherReferences'
    } as const)
    const {fields: otherArtists, append: appendArtis, remove: removeArtis} = useFieldArray({
        control: form.control,
        name: 'artists'
    } as const)
    ...
    const onSubmit: SubmitHandler<FormScore> = (data, e) => console.log("Submit", data, e)
            ...
            <CardContent>
                <Form {...form}>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)}>
                        ...
                        <TextInput control={form.control} name="reference.value"
                        ...
                        <FormItem className="flex gap-2 items-center">
                            ...
                            <Button type="button" variant="outline" size="sm" onClick={() => appendReference({value:"", information: ""})}><PlusCircleIcon /></Button>
                        </FormItem>

                        {otherReferences.map((ref, index: number) => (
                            <div key={`score_reference_${ref.id}`} className="flex gap-5 items-center">
                                <TextInput classname="flex gap-2 items-center flex-1" control={form.control} name={`otherReferences.${index}.value`} label={trans('entity.score.fields.refs.value.label')} />
                                <TextInput classname="flex gap-2 items-center flex-1" control={form.control} name={`otherReferences.${index}.information`} label={trans('entity.score.fields.refs.information.label')} />
                                <Button type="button" variant="outline" size="sm" className="w-[80px] text-red-800 font-semibold" onClick={() =>  removeReference(index)}><MinusSquareIcon /></Button>
                            </div>
                        ))}
                        ...
                        <Button type="submit" className="mt-4">{trans('main.action.save.label')}</Button>
                    </form>
                </Form>
            </CardContent>
        </Card>
